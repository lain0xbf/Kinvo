...
export function CarteiraModal({ userName }: ModalProps) {
    ...
    const header = (
        <View style={{ alignItems: 'center', paddingHorizontal: 24, paddingTop: 8 }}>
            <View style={{ width: 64, height: 64, borderRadius: 32, backgroundColor: '#FEF2F2', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                <Ionicons name="wallet-outline" size={32} color="#EA1D2C" />
            </View>
    ...
    return (
        <>
            <TouchableOpacity className="flex-row items-center justify-between px-4 py-4" onPress={() => setModalVisible(true)}>
                <View className="flex-row items-center">
                    <Ionicons name="wallet-outline" size={24} color="#0A2533" />
                    <View className="ml-3">
                        <Text style={{ fontFamily: 'SofiaProBold', color: '#0A2533' }} className="text-sm">Carteira</Text>
                        <Text style={{ fontFamily: 'SofiaProRegular', color: '#748189' }} className="text-xs">Meu saldo e QR code</Text>
                    </View>
                </View>
