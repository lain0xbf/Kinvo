...
function formatarValorInicial(valor: number) {
  return valor.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });
  ...
  return (
    ...
    >
      ...
      <AppText
        ...
        numberOfLines={1}
        variant="categoria"
        className={ativo ? 'mt-1.5 max-w-[54px] text-center text-emerald-700' : 'mt-1.5 max-w-[54px] text-center text-slate-500'}
      >
        {nome}
      </AppText>
    </Pressable>
...
export default function EditarTransacao() {
  ...
  useEffect(() => {
    async function carregarTransacao() {
      ...
      try {
        const transacao = await buscarTransacaoDoUsuario(userId, id);

        if (!transacao) {
          setErro('Transacao nao encontrada.');
          return;
        }

        setDescricao(transacao.descricao);
        setValorTexto(formatarValorInicial(transacao.valor));
        setCategoria(transacao.categoria);
        setTipo(transacao.tipo);
      } catch {
        setErro('Nao foi possivel carregar a transacao.');
      } finally {
        setCarregando(false);
      }
    }
  ...
  async function handleSalvarTransacao() {
    ...
    try {
      await atualizarTransacaoDoUsuario(userId, id, {
        descricao,
        valor,
        categoria,
        tipo,
      ...
      setErro('Nao foi possivel atualizar a transacao.');
    } finally {
      setSalvando(false);
    }
  }

  if (!fontsLoaded || !userId || carregando) {
      ...
      >
        <View className="mb-4 h-[76px] items-center justify-center py-2">
          ...
          <AppText variant="screenSubtitle" weight="regular" family="inter" className="mt-1 text-center text-slate-500">
            Atualize os dados do lancamento
          </AppText>
        </View>
        ...
        <View className="mt-3">
          ...
          <SurfaceCard variant="control" className="mt-3 flex-row items-center rounded-[26px] border-slate-200 bg-white px-6 py-4">
            <AppText family="inter" weight="regular" variant="fieldLabel" className="mr-2 text-slate-400">
              R$
            </AppText>
          ...
          <FormField label="Descricao" leftIcon={<Ionicons name="document-text-outline" size={22} color="#64748B" />}>
            <TextInput
              value={descricao}
        ...
        <Pressable
          ...
          accessibilityRole="button"
          accessibilityLabel={salvando ? 'Salvando transacao' : 'Salvar alteracoes'}
          ...
          {({ pressed }) => (
            ...
            >
              ...
              <View className="h-10 w-10 items-center justify-center rounded-[14px] bg-white">
                <Ionicons name={salvando ? 'hourglass-outline' : 'checkmark'} size={24} color="#10B981" />
              </View>
              ...
              <View className="ml-3 flex-1">
                ...
                <AppText family="sofia" variant="subcaption" className="mt-0.5 text-white/75">
                  Atualizar lancamento na carteira
                </AppText>
              </View>
